...
class InviteMemberTest < ActiveSupport::TestCase
  ...
  should 'dont require friend email if target given (person being invited)' do
    task = InviteMember.new(:target => create(:person))
    task.valid?
  ...
  should 'not send e-mails to requestor' do
    ...
    task = InviteMember.create!(:person => p1, :friend => p2, :community_id => create(Community).id)
    task.finish
  end
  ...
  should 'send e-mails to friend if friend_email given' do
    ...
    task = InviteMember.create!(:person => p1, :friend_email => 'test@test.com', :message => '<url>', :community_id => create(Community).id)
  end
