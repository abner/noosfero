...
class NotifyActivityToProfilesJobTest < ActiveSupport::TestCase
  ...
  should 'notify just the community in tracker with add_member_in_community verb' do
    ...
    action_tracker = create(ActionTracker::Record, :user_type => 'Profile', :user_id => person.id, :target_type => 'Profile', :target_id => community.id, :verb => 'add_member_in_community')
    assert NotifyActivityToProfilesJob::NOTIFY_ONLY_COMMUNITY.include?(action_tracker.verb)
    p1, p2, m1, m2 = create(Person), create(Person), create(Person), create(Person)
    create(Friendship, :person_id => person.id, :friend_id => p1.id)
    create(Friendship, :person_id => person.id, :friend_id => p2.id)
    create(RoleAssignment, :accessor_id => m1.id, :role_id => 3, :resource_id => community.id)
    create(RoleAssignment, :accessor_id => m2.id, :role_id => 3, :resource_id => community.id)
  ...
  should 'notify just the users and his friends tracking user actions' do
    person = create(Person)
    community  = create(Community)
    action_tracker = create(ActionTracker::Record, :user_type => 'Profile', :user_id => person.id, :target_type => 'Profile', :verb => 'create_article')
    assert !NotifyActivityToProfilesJob::NOTIFY_ONLY_COMMUNITY.include?(action_tracker.verb)
    p1, p2, m1, m2 = create(Person), create(Person), create(Person), create(Person)
    create(Friendship, :person_id => person.id, :friend_id => p1.id)
    create(Friendship, :person_id => person.id, :friend_id => p2.id)
    create(Friendship, :person_id => p1.id, :friend_id => m1.id)
    create(RoleAssignment, :accessor_id => m2.id, :role_id => 3, :resource_id => community.id)
    ActionTrackerNotification.delete_all
    job = NotifyActivityToProfilesJob.new(action_tracker.id)
    job.perform
  ...
  should 'not notify the communities members' do
    person = create(Person)
    community  = create(Community)
    action_tracker = create(ActionTracker::Record, :user_type => 'Profile', :user_id => person.id, :target_type => 'Profile', :target_id => community.id, :verb => 'create_article')
    assert !NotifyActivityToProfilesJob::NOTIFY_ONLY_COMMUNITY.include?(action_tracker.verb)
    m1, m2 = create(Person), create(Person), create(Person), create(Person)
    create(RoleAssignment, :accessor_id => m1.id, :role_id => 3, :resource_id => community.id)
    create(RoleAssignment, :accessor_id => m2.id, :role_id => 3, :resource_id => community.id)
    ActionTrackerNotification.delete_all
    job = NotifyActivityToProfilesJob.new(action_tracker.id)
  ...
  should 'notify only the community if it is private' do
    ...
    action_tracker = create(ActionTracker::Record, :user_type => 'Profile', :user_id => person.id, :target_type => 'Profile', :target_id => private_community.id, :verb => 'create_article')
    assert !NotifyActivityToProfilesJob::NOTIFY_ONLY_COMMUNITY.include?(action_tracker.verb)
    p1, p2, m1, m2 = create(Person), create(Person), create(Person), create(Person)
    create(Friendship, :person_id => person.id, :friend_id => p1.id)
    create(Friendship, :person_id => person.id, :friend_id => p2.id)
    create(RoleAssignment, :accessor_id => m1.id, :role_id => 3, :resource_id => private_community.id)
    create(RoleAssignment, :accessor_id => m2.id, :role_id => 3, :resource_id => private_community.id)
    ActionTrackerNotification.delete_all
    job = NotifyActivityToProfilesJob.new(action_tracker.id)
    job.perform
    process_delayed_job_queue
  ...
  should 'not notify the community tracking join_community verb' do
    person = create(Person)
    community  = create(Community)
    action_tracker = create(ActionTracker::Record, :user_type => 'Profile', :user_id => person.id, :target_type => 'Profile', :target_id => community.id, :verb => 'join_community')
    assert !NotifyActivityToProfilesJob::NOTIFY_ONLY_COMMUNITY.include?(action_tracker.verb)
    p1, p2, m1, m2 = create(Person), create(Person), create(Person), create(Person)
    create(Friendship, :person_id => person.id, :friend_id => p1.id)
    create(Friendship, :person_id => person.id, :friend_id => p2.id)
  ...
  should 'cancel notify when target no more exists' do
    ...
    create(Friendship, :person_id => person.id, :friend_id => friend.id)
    action_tracker = create(ActionTracker::Record, :user_type => 'Profile', :user_id => person.id, :target_type => 'Profile', :verb => 'create_article')
    ActionTrackerNotification.delete_all
    job = NotifyActivityToProfilesJob.new(action_tracker.id)
    person.destroy
