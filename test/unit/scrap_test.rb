...
class ScrapTest < ActiveSupport::TestCase
  ...
  should "collect all scraps sent and received of a community" do
    ...
    s2 = create(Scrap, :receiver_id => community.id, :sender_id => person.id)
    assert_equal [s2], Scrap.all_scraps(community)
    s3 = create(Scrap, :receiver_id => community.id)
  ...
  should "get replies of a scrap" do
    s = create(Scrap)
    s1 = create(Scrap, :scrap_id => s.id)
    s2 = create(Scrap)
    s3 = create(Scrap, :scrap_id => s.id)
    assert_equal [s1,s3], s.replies
  end

  should "get only replies scrap" do
    s0 = create(Scrap)
    s1 = create(Scrap, :scrap_id => s0.id)
    s2 = create(Scrap)
    s3 = create(Scrap, :scrap_id => s0.id)
  ...
  should "update the scrap on reply creation" do
    person = create(:person)
    s = create(Scrap, :updated_at => DateTime.parse('2010-01-01'))
