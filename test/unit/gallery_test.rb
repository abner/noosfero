...
class GalleryTest < ActiveSupport::TestCase
  ...
  should 'have images that are only images or other galleries' do
    ...
    gallery = create(Gallery, :profile_id => p.id, :parent_id => f.id)
  ...
  should 'bring galleries first in alpha order in images listing' do
    p = create(:person)
    f = create(Gallery, :profile_id => p.id)
    gallery1 = create(Gallery, :name => 'b', :profile_id => p.id, :parent_id => f.id)
    image = create(UploadedFile, :uploaded_data => fixture_file_upload('/files/rails.png', 'image/png'), :parent => f, :profile => p)
    gallery2 = create(Gallery, :name => 'c', :profile_id => p.id, :parent_id => f.id)
    gallery3 = create(Gallery, :name => 'a', :profile_id => p.id, :parent_id => f.id)
  ...
  should 'return newest text articles as news' do
    c = create(Community)
    gallery = create(Gallery, :profile_id => c.id)
    f = create(Gallery, :name => 'gallery', :profile_id => c.id, :parent_id => gallery.id)
    u = create(UploadedFile, :profile => c, :uploaded_data => fixture_file_upload('/files/rails.png', 'image/png'), :parent => gallery)
    older_t = create(TinyMceArticle, :name => 'old news', :profile_id => c.id, :parent_id => gallery.id)
    t = create(TinyMceArticle, :name => 'news', :profile_id => c.id, :parent_id => gallery.id)
  ...
  should 'not return highlighted news when not asked' do
    c = create(Community)
    gallery = create(Gallery, :profile_id => c.id)
    highlighted_t = create(TinyMceArticle, :name => 'high news', :profile_id => c.id, :highlighted => true, :parent_id => gallery.id)
    t = create(TinyMceArticle, :name => 'news', :profile_id => c.id, :parent_id => gallery.id)
